...
def register_session(session_path, settings_dict, one=None):
    ...
    log.info(
        f'session registered in Alyx database: {ses["subject"]}, {ses["start_time"]}, {ses["number"]}'
    )

    # add the weight if available and if there is no previous weighing registered
    if settings_dict["SUBJECT_WEIGHT"]:
        wd = dict(
            nickname=settings_dict["SUBJECT_NAME"],
        ...
        if len(previous_weighings) == 0:
            ...
            log.info(
                f"weighing registered in Alyx database: {ses['subject']}, {settings_dict['SUBJECT_WEIGHT']}g"
            )
