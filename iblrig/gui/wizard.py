...
from iblrig.choiceworld import (
...
def _set_list_view_from_string_list(ui_list: QtWidgets.QListView, string_list: list):
    ...
    for i, s in enumerate(ui_list.model().stringList()):
        if s in string_list:
            ui_list.selectionModel().select(
                ui_list.model().createIndex(i, 0), QtCore.QItemSelectionModel.Select
            )
...
class RigWizardModel:
    ...
    def __post_init__(self):
        ...
        self.all_users = (
            [self.iblrig_settings["ALYX_USER"]]
            if self.iblrig_settings["ALYX_USER"]
            else []
        )
        ...
        if CUSTOM_TASKS:
            tasks.extend(
                sorted(
                    [
                        p
                        for p in Path(iblrig_custom_tasks.__file__).parent.rglob(
                            "task.py"
                        )
                    ]
                )
        ...
        if self.iblrig_settings["iblrig_local_data_path"] is None:
            ...
            ).joinpath(self.iblrig_settings["ALYX_LAB"], "Subjects")
            self.all_subjects = [self.test_subject_name] + sorted(
                [
                    f.name
                    for f in folder_subjects.glob("*")
                    if f.is_dir() and f.name != self.test_subject_name
                ]
            )
    ...
    def connect(self, username=None, one=None, gui=False):
        ...
        rest_subjects = self.one.alyx.rest(
            "subjects",
            "list",
            alive=True,
            stock=False,
        ...
        projects = [
            ...
            if (username in p["users"] or len(p["users"]) == 0)
        ]
...
class RigWizard(QtWidgets.QMainWindow, Ui_wizard):
    def __init__(self, *args, **kwargs):
        ...
        self.uiPushStatusLED.setChecked(
            self.settings.value("bpod_status_led", True, bool)
        )
        ...
        local_data = (
            Path(local_data) if local_data else Path.home().joinpath("iblrig_data")
        )
        ...
        self.uiProgressDiskSpace.setStatusTip(
            f"utilization of drive {local_data.anchor}"
        )
    ...
    def _on_menu_training_level_v7(self) -> None:
        ...
        if session_path is None or session_path == "":
            return
        file_jsonable = next(
            Path(session_path).glob("raw_behavior_data/_iblrig_taskData.raw.jsonable"),
            None,
        )
        if file_jsonable is None:
            QtWidgets.QMessageBox().critical(
                self, "Error", f"No jsonable found in {session_path}"
            )
            return
        trials_table, _ = iblrig.raw_data_loaders.load_task_jsonable(file_jsonable)
        if trials_table.empty:
            QtWidgets.QMessageBox().critical(
                self, "Error", f"No trials found in {session_path}"
            )
            return
        ...
        if self.uiComboTask.currentText() == "_iblrig_tasks_trainingChoiceWorld":
            box.setStandardButtons(
                QtWidgets.QMessageBox.Apply | QtWidgets.QMessageBox.Close
            )
        else:
            box.setStandardButtons(QtWidgets.QMessageBox.Close)
        box.exec()
        if box.clickedButton() == box.button(QtWidgets.QMessageBox.Apply):
            self.uiGroupTaskParameters.findChild(
                QtWidgets.QWidget, "--adaptive_gain"
            ).setValue(stim_gain)
            self.uiGroupTaskParameters.findChild(
                QtWidgets.QWidget, "--adaptive_reward"
            ).setValue(reward_amount)
            self.uiGroupTaskParameters.findChild(
                QtWidgets.QWidget, "--training_phase"
            ).setValue(training_phase)
    ...
    def _on_check_dirty_result(self, repository_is_dirty: bool) -> None:
        ...
        if repository_is_dirty:
            ...
            msg_box.setText(
                "Your copy of iblrig contains local changes.\nDon't expect things to work as intended!"
            )
    ...
    def eventFilter(self, obj, event):
        if obj == self.uiPushStart and event.type() in [
                ...
                match event.type():
                    case QtCore.QEvent.HoverEnter:
                        widget.setStyleSheet(
                            "QListView { background-color: pink; border: 1px solid red; }"
                        )
                    case _:
                        widget.setStyleSheet("")
            return True
    ...
    def model2view(self):
        ...
        self.uiComboTask.setModel(
            QtCore.QStringListModel(list(self.model.all_tasks.keys()))
        )
        self.uiComboSubject.setModel(QtCore.QStringListModel(self.model.all_subjects))
        self.uiListProcedures.setModel(
            QtCore.QStringListModel(self.model.all_procedures)
        )
    ...
    def controller2model(self):
        self.model.procedures = [
            i.data() for i in self.uiListProcedures.selectedIndexes()
        ]
    ...
    def controls_for_extra_parameters(self):
        ...
        args = (
            sorted(
                self.model.get_task_extra_parser(self.model.task_name)._actions,
                key=lambda x: x.dest,
            )
            + args
        )
        ...
        for arg in args:
            ...
            if isinstance(arg, argparse._StoreTrueAction | argparse._StoreFalseAction):
                ...
                widget.toggled.connect(
                    lambda val, p=param: self._set_task_arg(p, val > 0)
                )
                ...
                if isinstance(arg.choices, list):
                    ...
                    if arg.default:
                        widget.setCurrentIndex(
                            [widget.itemText(x) for x in range(widget.count())].index(
                                arg.default
                            )
                        )
            ...
            # create widget for list of floats
                ...
                widget.editingFinished.connect(
                    lambda p=param, w=widget: self._set_task_arg(
                        p, [x.strip() for x in w.text().split(",")]
                    )
                )
                widget.editingFinished.emit()
    ...
    def pause(self):
        self.uiPushPause.setStyleSheet(
            "QPushButton {background-color: yellow;}"
            if self.uiPushPause.isChecked()
            else ""
        )
        ...
        match self.uiPushStart.text():
            case "Start":
                ...
                task = EmptySession(
                    ...
                    append=self.uiCheckAppend.isChecked(),
                    interactive=False,
                )
                ...
                for key in self.task_arguments:
                    if isinstance(
                        self.task_arguments[key], Iterable
                    ) and not isinstance(self.task_arguments[key], str):
                        cmd.extend([str(key)])
                ...
                self.uiPushStart.setStatusTip(
                    "stop the session after the current trial"
                )
                self.uiPushStart.setIcon(self.style().standardIcon(QStyle.SP_MediaStop))
            ...
            case "Stop":
                ...
                if (
                    task_settings_file := Path(self.model.raw_data_folder).joinpath(
                    ...
                    ) < 42 and "spontaneous" not in self.model.task_name:
                    ...
                    with open(task_settings_file, "w") as fid:
                        json.dump(
                            session_data, fid, indent=4, sort_keys=True, default=str
                        )
    ...
    def check_sub_process(self):
        return_code = (
            None
            if self.running_task_process is None
            else self.running_task_process.poll()
        )
    ...
    def flush(self):
        ...
        try:
            ...
            bpod.manual_override(
                bpod.ChannelTypes.OUTPUT,
                bpod.ChannelNames.VALVE,
                1,
                self.uiPushFlush.isChecked(),
            )
    ...
    def toggle_status_led(self, is_toggled: bool):
        ...
        self.uiPushStatusLED.setStyleSheet(
            "QPushButton {background-color: rgb(128, 255, 128);}" if is_toggled else ""
        )
        self.enable_UI_elements()
