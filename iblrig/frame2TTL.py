    ...
    def connect(self):
        ...
        if not self.connected:
            self.serial = serial.Serial(
                port=self.serial_port, baudrate=115200, timeout=3.0, write_timeout=1.0
            )
...
class Frame2TTLv1(Frame2TTL):
    ...
    def handshake(self):
        ...
        handshakeByte = int.from_bytes(
            self.serial.read(1), byteorder="little", signed=False
        )
...
class Frame2TTLv2(Frame2TTL):
    ...
    def connect(self):
        ...
        try:
            ...
            self.serial.write(b"#")
            # 1 byte response expected (unsigned)
            self.hw_version = int.from_bytes(
                self.serial.read(1), byteorder="little", signed=False
            )
    ...
    def read_sensor(self, nsamples: int = 1) -> int:
        ...
        self.serial.write(
            b"V" + int.to_bytes(nsamples, 4, byteorder="little", signed=False)
        )
    ...
    def measure_black(self, mode="auto"):
        ...
        if mode == "auto":
            ...
            threshold = int.from_bytes(
                self.serial.read(2), byteorder="little", signed=True
            )
    ...
    def calc_recomend_thresholds(self):
        ...
        """
        manual_calib_run = (
            self.manual_dark is not None and self.manual_light is not None
        )
        ...
        if not auto_calib_run and not manual_calib_run:
            log.error(
                "Please run .measure_white and .measure_black to recalculate thresholds."
            )
            return -1
        ...
        if self.auto_dark > self.auto_light:
            ...
            log.info(f"Light ={self.recomend_light}, Dark = {self.recomend_dark}.")
            log.warning(
                "Recommended thresholds not set yet. Please call set_recommendations()"
            )
            return self.recomend_dark, self.recomend_light
