...
def _transfer_parser(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="dry",
        help="do not remove local data after copying",
    )
    return parser
...
def _get_subjects_folders(
    local_path: Path, remote_path: Path, interactive: bool = False
) -> tuple[Path, Path]:
...
def _get_copiers(
    ...
    rig_paths = get_local_and_remote_paths(
        local_path=local_folder, remote_path=remote_folder, lab=lab
    )
    ...
    # get copiers
    copiers = [
        copier(f.parent, remote_subjects_folder)
...
def transfer_ephys_data(
    local_path: Path = None,
    remote_path: Path = None,
    dry: bool = False,
    interactive: bool = False,
):
    local_subject_folder, remote_subject_folder = _get_subjects_folders(
        local_path, remote_path, interactive
    )
    copiers = _get_copiers(
        EphysCopier, local_path, remote_path, interactive=interactive
    )
    ...
    remove_local_sessions(
        weeks=2,
        local_path=local_subject_folder,
        remote_path=remote_subject_folder,
        dry=dry,
        tag="ephys",
    )


def transfer_video_data(
    local_path: Path = None,
    remote_path: Path = None,
    dry: bool = False,
    interactive: bool = False,
    ...
    copiers = _get_copiers(
        VideoCopier, local_path, remote_path, interactive=interactive
    )
...
def transfer_data(
    ...
    copiers = _get_copiers(
        BehaviorCopier, local_path, remote_path, interactive=interactive
    )

    for copier in copiers:
        session_path = copier.session_path
        task_settings = raw_data_loaders.load_settings(
            session_path, task_collection="raw_task_data_00"
        )
        ...
        if task_settings["SESSION_END_TIME"] is None:
            jsonable = session_path.joinpath(
            ...
            settings_file = session_path.joinpath(
            ...
            end_time = datetime.datetime.strptime(
                raw_settings["SESSION_START_TIME"], "%Y-%m-%dT%H:%M:%S.%f"
            )
...
def remove_local_sessions(
    weeks=2, local_path=None, remote_path=None, dry=False, tag="behavior"
):
    """
    ...
    match tag:
        ...
        case "video":
            Copier = VideoCopier
    for flag in sorted(
        list(local_subject_folder.rglob(f"_ibl_experiment.description_{tag}.yaml")),
        reverse=True,
    ):
        session_path = flag.parent
        days_elapsed = (
            datetime.datetime.now()
            - datetime.datetime.strptime(session_path.parts[-2], "%Y-%m-%d")
        ).days
        ...
        if sc.state == 3:
            session_size = (
                sum(f.stat().st_size for f in session_path.rglob("*") if f.is_file())
                / 1024**3
            )
...
def flush():
    ...
    """
    file_settings = (
        Path(iblrig.__file__).parents[1].joinpath("settings", "hardware_settings.yaml")
    )
