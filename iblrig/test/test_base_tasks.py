...
class TestHierarchicalParameters(unittest.TestCase):
    def test_default_params(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            sess2 = BiasedChoiceWorldSession(
                task_parameter_file=file_params, **TASK_KWARGS
            )
...
class TestHardwareMixins(unittest.TestCase):
    def setUp(self):
        ...
        self.session = EmptyHardwareSession(
            task_parameter_file=task_settings_file, **TASK_KWARGS
        )
...
class TestExperimentDescription(unittest.TestCase):
    ...
    def test_stub(self):
        ...
        expected = self.stub["tasks"] + [
            {"passiveWorld": {"collection": "raw_task_data_00", "sync_label": "bpod"}}
        ]
...
class TestPathCreation(unittest.TestCase):
    ...
    def test_create_chained_protocols(self):
        ...
        self.assertEqual(
            set(
                [
                    d.name
                    for d in first_task.paths.SESSION_FOLDER.iterdir()
                    if d.is_dir()
                ]
            ),
        ...
        protocols = set(
            [
                p[EmptyHardwareSession.protocol_name]["collection"]
                for p in description["tasks"]
            ]
        )
    ...
    def test_create_session_with_remote(self):
        with tempfile.TemporaryDirectory() as td:
            task = EmptyHardwareSession(
                iblrig_settings={"iblrig_remote_data_path": Path(td)}, **TASK_KWARGS
            )
            task.create_session()
            # when we create the session, the local session folder is created with the acquisition description file
            description_file_local = next(
                task.paths["SESSION_FOLDER"].glob("_ibl_experiment.description*.yaml"),
                None,
            )
            remote_session_path = task.paths["REMOTE_SUBJECT_FOLDER"].joinpath(
                task.paths["SESSION_FOLDER"].relative_to(
                    task.paths["LOCAL_SUBJECT_FOLDER"]
                )
            )
            # there is also the acquisition description stub in the remote folder
            description_file_remote = next(
                remote_session_path.joinpath("_devices").glob("*.yaml"), None
            )
    ...
    def test_create_session_without_remote(self):
        task = EmptyHardwareSession(
            iblrig_settings={"iblrig_remote_data_path": None}, **TASK_KWARGS
        )
        task.create_session()
        # when we create the session, the local session folder is created with the acquisition description file
        description_file_local = next(
            task.paths["SESSION_FOLDER"].glob("_ibl_experiment.description*.yaml"), None
        )
        assert description_file_local is not None

    def test_create_session_unavailable_remote(self):
        task = EmptyHardwareSession(
            iblrig_settings={"iblrig_remote_data_path": "/path/that/doesnt/exist"},
            **TASK_KWARGS,
        )
        task.create_session()
        # when we create the session, the local session folder is created with the acquisition description file
...
class TestTaskArguments(unittest.TestCase):
    ...
    def test_arg_parser_user(self):
        ...
        kwargs = self._parse_local(
            args=["--subject", "toto", "--training_phase", "4"], parents=[parser]
        )
        self.assertTrue(kwargs["interactive"])
        self.assertEqual(kwargs["subject"], "toto")
