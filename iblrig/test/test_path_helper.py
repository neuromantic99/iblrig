...
class TestPathHelper(unittest.TestCase):
    ...
    def test_get_commit_hash(self):
        ...
        # Run it
        ch = iblrig.path_helper.get_commit_hash(
            str(iblrig.path_helper.get_iblrig_path())
        )
...
class TestIterateCollection(unittest.TestCase):
    ...
    def test_iterate_collection(self):
        ...
        next_collection = iblrig.path_helper.iterate_collection(
            str(self.session_path), "raw_foo_data"
        )
        self.assertEqual("raw_foo_data_04", next_collection)
        next_collection = iblrig.path_helper.iterate_collection(
            str(self.session_path), "raw_bar_data"
        )
...
class TestPatchSettings(unittest.TestCase):
    ...
    def test_patch_hardware_settings(self):
        rs = {
            "RIG_NAME": "foo_rig",
            "MAIN_SYNC": True,
            "device_camera": {"BONSAI_WORKFLOW": "path/to/Workflow.bonsai"},
        }
        ...
        self.assertEqual(
            rs["device_camera"], updated.get("device_cameras", {}).get("left")
        )
        self.assertDictEqual(
            iblrig.path_helper.patch_settings(updated.copy(), "hardware_settings"),
            updated,
        )
...
class TestHardwareSettings(unittest.TestCase):
    def test_get_left_camera_workflow(self):
        ...
        self.assertIsNotNone(
            BonsaiRecordingMixin._camera_mixin_bonsai_get_workflow_file(
                hws["device_cameras"]
            )
        )
        self.assertIsNone(
            BonsaiRecordingMixin._camera_mixin_bonsai_get_workflow_file(None)
        )
        self.assertIsNone(
            BonsaiRecordingMixin._camera_mixin_bonsai_get_workflow_file({})
        )
