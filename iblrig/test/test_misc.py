...
class TestMisc(unittest.TestCase):
    def test_draw_contrast(self):
        ...
        def assert_distribution(
            values: list[int], f_exp: list[float] | None = None
        ) -> None:
        ...
        contrasts = [
            misc.draw_contrast(contrast_set, "uniform") for i in range(n_draws)
        ]
        ...
        for p_idx in [0.25, 0.5, 0.75, 1.25]:
            contrasts = [
                misc.draw_contrast(contrast_set, "biased", 0, p_idx)
        ...
        self.assertRaises(
            ValueError, misc.draw_contrast, [], "incorrect_type"
        )  # assert exception for incorrect type
        self.assertRaises(
            IndexError, misc.draw_contrast, [0, 1], "biased", 2
        )  # assert exception for out-of-range index
    ...
    def test_online_std(self):
        ...
        a = b[:-1]
        mu, std = online_std(
            new_sample=b[-1], new_count=n, old_mean=np.mean(a), old_std=np.std(a)
        )
