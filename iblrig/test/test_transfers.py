...
def _create_behavior_session(temp_dir, ntrials=None, hard_crash=False):
    ...
    if ntrials is not None:
        with open(
            Path(__file__).parent.joinpath("fixtures", "task_data_short.jsonable")
        ) as fid:
            lines = fid.readlines()
        ...
        if not hard_crash:
            ...
            session.session_info["SESSION_END_TIME"] = session.session_info[
                "SESSION_START_TIME"
            ]
    ...
    def test_behavior_copy_complete_session(self):
        ...
        for hard_crash in [False, True]:
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as td:
                session = _create_behavior_session(
                    td, ntrials=50, hard_crash=hard_crash
                )
                session.paths.SESSION_FOLDER.joinpath("transfer_me.flag").touch()
                with mock.patch(
                    "iblrig.path_helper.load_settings_yaml",
                    return_value=session.iblrig_settings,
                ):
...
class TestUnitTransferExperiments(unittest.TestCase):
    ...
    def test_behavior_ephys_video_copy(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            folder_session_video = Path(td).joinpath(
                "video", "Subjects", *session.paths.SESSION_FOLDER.parts[-3:]
            )
            folder_session_ephys = Path(td).joinpath(
                "ephys", "Subjects", *session.paths.SESSION_FOLDER.parts[-3:]
            )
            ...
            for i in range(2):
                ...
                folder_probe = folder_session_ephys.joinpath(
                    "raw_ephys_data", "_spikeglx_ephysData_g0", pname
                )
            ...
            folder_session_ephys.joinpath(
                "raw_ephys_data",
                "_spikeglx_ephysData_g0",
                "_spikeglx_ephysData_g0_t0.imec0.nidq.bin",
            ).touch()
            folder_session_ephys.joinpath(
                "raw_ephys_data",
                "_spikeglx_ephysData_g0",
                "_spikeglx_ephysData_g0_t0.imec0.nidq.meta",
            ).touch()
            """
            ...
            for vname in ["body", "left", "right"]:
                folder_session_video.joinpath(
                    "raw_video_data", f"_iblrig_{vname}Camera.frameData.bin"
                ).touch()
                folder_session_video.joinpath(
                    "raw_video_data", f"_iblrig_{vname}Camera.raw.avi"
                ).touch()
            """
            Test the copiers
            """
            ...
            assert (
                sc.state == 2
            )  # here we still don't have all devides so this won't cut it and we stay in state 2

            vc = VideoCopier(
                session_path=folder_session_video,
                remote_subjects_folder=session.paths.REMOTE_SUBJECT_FOLDER,
            )
            ...
            ec = EphysCopier(
                session_path=folder_session_ephys,
                remote_subjects_folder=session.paths.REMOTE_SUBJECT_FOLDER,
            )
            assert ec.state == 0
            ...
            sc.finalize_copy(number_of_expected_devices=3)
            assert sc.state == 3
            final_experiment_description = session_params.read_params(
                sc.remote_session_path
            )
            assert len(final_experiment_description["tasks"]) == 1
            assert set(
                final_experiment_description["devices"]["cameras"].keys()
            ) == set(["body", "left", "right"])
