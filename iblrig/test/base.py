    ...
    class CommonTestInstantiateTask(CommonTestTask):
        ...
        def test_graphviz(self) -> None:
            if hasattr(self.task, "get_graphviz_task"):
                self.task.mock()
                pdf_out = Path(inspect.getfile(self.task.__class__)).parent.joinpath(
                    "state_machine_graph"
                )
        ...
        def test_acquisition_description(self) -> None:
            ...
            description_task = self.task.experiment_description["tasks"][0][
                self.task.protocol_name
            ]
...
class IntegrationFullRuns(BaseTestCases.CommonTestTask):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.one.alyx.rest(
            "subjects",
            "create",
            data=dict(nickname=cls.kwargs["subject"], lab="cortexlab"),
        )
