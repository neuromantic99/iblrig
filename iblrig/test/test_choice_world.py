...
from iblrig_tasks._iblrig_tasks_passiveChoiceWorld.task import (
    Session as PassiveChoiceWorldSession,
)
from iblrig_tasks._iblrig_tasks_spontaneous.task import Session as SpontaneousSession
from iblrig_tasks._iblrig_tasks_trainingChoiceWorld.task import (
    Session as TrainingChoiceWorldSession,
)
...
class TestGetPreviousSession(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.kwargs["iblrig_settings"] = dict(
            iblrig_local_data_path=self.root_path, ALYX_LAB="cortexlab"
        )
    ...
    def test_iterate_previous_sessions(self):
        ...
        with tempfile.TemporaryDirectory() as tdd:
            shutil.copytree(
                self.root_path.joinpath("cortexlab"), tdd, dirs_exist_ok=True
            )
            ...
            self.assertEqual(
                len(set([ps["session_path"].parents[3] for ps in previous_sessions])), 2
            )

    @staticmethod
    def mock_jsonable(file_path, training_phase=3, reward_amount=None):
        file_fixtures = Path(__file__).parent.joinpath(
            "fixtures", "task_data_short.jsonable"
        )
    ...
    def test_adaptive_training_level(self):
        ...
        """
        self.mock_jsonable(
            self.sesb.paths.DATA_FILE_PATH, training_phase=2, reward_amount=1050
        )
        ...
        t = TrainingChoiceWorldSession(
            **self.kwargs, training_phase=4, adaptive_reward=2.9, adaptive_gain=6.0
        )
        result = (
            t.training_phase,
            t.session_info["ADAPTIVE_REWARD_AMOUNT_UL"],
            t.session_info["ADAPTIVE_GAIN_VALUE"],
        )
        ...
        self.sesb.save_task_parameters_to_json_file()
        self.mock_jsonable(
            self.sesb.paths.DATA_FILE_PATH, training_phase=1, reward_amount=500
        )
    ...
    def test_adaptive_reward(self):
        ...
        for args, expected in fixture:
            ...
            with self.subTest(args=args):
                self.assertEqual(
                    expected, iblrig.choiceworld.compute_adaptive_reward_volume(*args)
                )
