...
from iblrig.version_management import (
    check_for_updates,
...
class TestCheckForUpdates(unittest.TestCase):
    @patch(
        ...
        return_value=version.parse("1.0.0"),
    )
    @patch(
...
class TestGetDetailedVersionString(unittest.TestCase):
    ...
    @patch(
        "iblrig.version_management.check_output",
        return_value="1.0.0-42-gfe39a9d2-dirty\n",
    )
    def test_detailed_version_string_generation(
        self, mock_check_output, mock_get_remote_tags, mock_internet_available
    ):
        with self.assertNoLogs("iblrig", level="ERROR"):
