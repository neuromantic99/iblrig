...
from iblrig_tasks._iblrig_tasks_ephysChoiceWorld.task import (
    Session as EphysChoiceWorldSession,
)
from iblrig_tasks._iblrig_tasks_ImagingChoiceWorld.task import (
    Session as ImagingChoiceWorldSession,
)
from iblrig_tasks._iblrig_tasks_neuroModulatorChoiceWorld.task import (
    Session as NeuroModulatorChoiceWorldSession,
)
    ...
    def test_task(self, reward_set=np.array([0, 1.5])):
        ...
        for i in np.arange(nt):
            ...
            trial_type = np.random.choice(
                ["correct", "error", "no_go"], p=[0.9, 0.05, 0.05]
            )
        ...
        np.testing.assert_array_equal(
            task.trials_table["trial_num"].values, np.arange(task.trial_num + 1)
        )
        # makes sure the water reward counts check out
        assert (
            task.trials_table["reward_amount"].sum()
            == task.session_info.TOTAL_WATER_DELIVERED
        )
        assert (
            np.sum(task.trials_table["reward_amount"] == 0)
            == task.trial_num + 1 - task.session_info.NTRIALS_CORRECT
        )
        ...
        df_blocks = task.trials_table.groupby("block_num").agg(
            ...
            n_stim_probability_left=pd.NamedAgg(
                column="stim_probability_left", aggfunc="nunique"
            ),
            stim_probability_left=pd.NamedAgg(
                column="stim_probability_left", aggfunc="first"
            ),
            position=pd.NamedAgg(
                column="position", aggfunc=lambda x: 1 - (np.mean(np.sign(x)) + 1) / 2
            ),
        ...
        assert np.all(
            np.isclose(
                np.abs(np.diff(df_blocks["stim_probability_left"].values[1:])), 0.6
            )
        )
        # assert the the trial outcomes are within 0.3 of the generating probability
        np.testing.assert_array_less(
            np.abs(df_blocks["position"] - df_blocks["stim_probability_left"]), 0.4
        )
        np.testing.assert_array_equal(
            np.unique(task.trials_table["reward_amount"]), reward_set
        )
...
class TestIntegrationFullRun(IntegrationFullRuns):
    ...
    def test_task_biased(self):
        ...
        task.run()
        file_settings = task.paths.SESSION_RAW_DATA_FOLDER.joinpath(
        ...
        trials_table, bpod_data = load_task_jsonable(
            task.paths.SESSION_RAW_DATA_FOLDER.joinpath("_iblrig_taskData.raw.jsonable")
        )
        ...
        self.assertEqual(
            set(full_session["procedures"]), set(self.kwargs["procedures"])
        )
        # and the water administered
        assert (
            full_session["wateradmin_session_related"][0]["water_administered"]
            == init_water / 1000
        )
