...
class ChoiceWorldParams(BaseModel):
    ...
    CONTRAST_SET: list[Probability] = Field(
        [1.0, 0.25, 0.125, 0.0625, 0.0], min_items=1
    )
    ...
    QUIESCENCE_THRESHOLDS: list[float] = Field(
        default=[-2, 2], min_length=2, max_length=2
    )
    ...
    STIM_TRANSLATION_Z: Literal[
        7, 8
    ] = 7  # 7 for ephys, 8 otherwise. -p:Stim.TranslationZ-{STIM_TRANSLATION_Z} bonsai parameter
    SYNC_SQUARE_X: float = 1.33
    SYNC_SQUARE_Y: float = -1.03
    USE_AUTOMATIC_STOPPING_CRITERIONS: bool = True
    VISUAL_STIMULUS: str = (
        "GaborIBLTask / Gabor2D.bonsai"  # null / passiveChoiceWorld_passive.bonsai
    )
...
class ChoiceWorldSession(
    ...
    base_parameters_file = Path(__file__).parent.joinpath(
        "base_choice_world_params.yaml"
    )
    ...
    def get_graphviz_task(self, output_file=None, view=True):
        ...
        if output_file is not None:
            try:
                ...
                log.info(
                    "Graphviz system executable not found, cannot render the graph"
                )
        return dot
    ...
    def draw_next_trial_info(self, pleft=0.5, contrast=None, position=None):
        if contrast is None:
            contrast = misc.draw_contrast(
                self.task_params.CONTRAST_SET,
                self.task_params.CONTRAST_SET_PROBABILITY_TYPE,
            )
        assert (
            len(self.task_params.STIM_POSITIONS) == 2
        ), "Only two positions are supported"
        position = position or int(
            np.random.choice(self.task_params.STIM_POSITIONS, p=[pleft, 1 - pleft])
        )
        quiescent_period = (
            self.task_params.QUIESCENT_PERIOD
        ...
        self.trials_table.at[self.trial_num, "stim_phase"] = random.uniform(
            0, 2 * math.pi
        )
    ...
    def get_state_machine_trial(self, i):
        ...
        if i == 0:  # First trial exception start camera
            ...
            sma.add_state(
                ...
                output_actions=[
                    self.bpod.actions.bonsai_hide_stim,
                    ("SoftCode", SOFTCODE.TRIGGER_CAMERA),
                    ("BNC1", 255),
                ],
            )  # sart camera
        else:
...
class ActiveChoiceWorldSession(ChoiceWorldSession):
    ...
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self.trials_table["stim_probability_left"] = np.zeros(
            NTRIALS_INIT, dtype=np.float32
        )
    ...
    def _run(self):
        ...
        if self.interactive:
            subprocess.Popen(
                ["viewsession", str(self.paths["DATA_FILE_PATH"])],
...
NTRIALS ERROR:        {self.trial_num - self.session_info.NTRIALS_CORRECT}
    ...
    def trial_completed(self, bpod_data):
        ...
        response_time = (
            bpod_data["States timestamps"]["closed_loop"][0][1]
        ...
        outcome = {
            sn: ~np.isnan(bpod_data["States timestamps"].get(sn, [[np.NaN]])[0][0])
...
class BiasedChoiceWorldSession(ActiveChoiceWorldSession):
    ...
    base_parameters_file = Path(__file__).parent.joinpath(
        "base_biased_choice_world_params.yaml"
    )
    ...
    def new_block(self):
        ...
        if self.block_num == 0:
            pleft = (
                ...
                else np.random.choice(self.task_params.BLOCK_PROBABILITY_SET)
            )
            ...
            pleft = round(
                abs(1 - self.blocks_table.loc[self.block_num - 1, "probability_left"]),
                1,
            )
    ...
    def next_trial(self):
        ...
        self.block_trial_num += 1
        if self.block_num < 0 or self.block_trial_num > (
            self.blocks_table.loc[self.block_num, "block_length"] - 1
        ):
            self.new_block()
...
class TrainingChoiceWorldSession(ActiveChoiceWorldSession):
    ...
    def __init__(
        self, training_phase=-1, adaptive_reward=-1.0, adaptive_gain=None, **kwargs
    ):
        super().__init__(**kwargs)
        (
            inferred_training_phase,
            inferred_adaptive_reward,
            inferred_adaptive_gain,
        ) = self.get_subject_training_info()
        if training_phase == -1:
        ...
        if adaptive_reward == -1:
            ...
            self.logger.critical(
                f"Adaptive reward manually set to {adaptive_reward} uL"
            )
            self.session_info["ADAPTIVE_REWARD_AMOUNT_UL"] = adaptive_reward
        if adaptive_gain is None:
            self.logger.critical(
                f"Got Adaptive gain {inferred_adaptive_gain} degrees/mm"
            )
            self.session_info["ADAPTIVE_GAIN_VALUE"] = inferred_adaptive_gain
        else:
            self.logger.critical(
                f"Adaptive gain manually set to {adaptive_gain} degrees/mm"
            )
    ...
    @property
    def reward_amount(self):
        return self.session_info.get(
            "ADAPTIVE_REWARD_AMOUNT_UL", self.task_params.REWARD_AMOUNT_UL
        )
    ...
    def get_subject_training_info(self):
        ...
        try:
            ...
            self.logger.critical(
                "Failed to get training information from previous subjects: %s",
    ...
    def compute_performance(self):
        ...
        :return: None
        """
        self.trials_table["signed_contrast"] = self.trials_table["contrast"] * np.sign(
            self.trials_table["position"]
        )
        performance = self.trials_table.groupby(["signed_contrast"]).agg(
            last_50_perf=pd.NamedAgg(
                column="trial_correct",
    ...
    def check_training_phase(self):
        ...
        ):  # each of the -1, -.5, .5, 1 contrast should be above 80% perf to switch
    ...
    def next_trial(self):
        ...
        if self.task_params.DEBIAS and self.trial_num >= 1 and self.training_phase < 5:
            ...
            if do_debias_trial:
                ...
                position = self.task_params.STIM_POSITIONS[
                    int(np.random.normal(average_right, 0.5) >= 0.5)
                ]
