...
class ValidateHardware(ABC):
    ...
    def __init__(self, iblrig_settings=None, hardware_settings=None):
        self.iblrig_settings = iblrig_settings or load_settings_yaml(
            "iblrig_settings.yaml"
        )
        self.hardware_settings = hardware_settings or load_settings_yaml(
    ...
    def _run(self) -> ValidateResult:
        if self.port is None:
            result = ValidateResult(
                "FAIL", f"No serial port defined for {self.device_name}"
            )
        elif (
            next((p for p in list_ports.comports() if p.device == self.port), None)
            is None
        ):
            ...
            try:
                ...
                result = ValidateResult(
                    "FAIL", f"`{self.port}` cannot be connected to", exception=e
                )
            else:
                result = ValidateResult(
                    "PASS",
                    f"`{self.port}` is a valid serial port that can be connected to",
                )
        # first, test for properties of the serial port without opening the latter (VID, PID, etc)
        passed = (
            self.port in filter_ports(**self.port_properties)
            if self.port_properties is not None
            else False
        )
        ...
        if passed:
            result = ValidateResult(
                "PASS",
                f"Device on `{self.port}` does in fact seem to be a {self.device_name}",
            )
        else:
            result = ValidateResult(
                "FAIL",
                f"Device on `{self.port}` does NOT seem to be a {self.device_name}",
            )
...
class ValidateAlyxLabLocation(ValidateHardware):
    ...
    def _run(self, one):
        try:
            ...
            error_message = (
                f'Could not find rig name {self.hardware_settings["RIG_NAME"]} in Alyx'
            )
            solution = (
            ...
            results_kwargs = dict(
                status="FAIL", message=error_message, solution=solution
            )
