...
def copy_folders(
    local_folder: Path, remote_folder: Path, overwrite: bool = False
) -> bool:
    """
...
class SessionCopier(abc.ABC):
    ...
    def __init__(self, session_path, remote_subjects_folder=None, tag=None):
        ...
        self.remote_subjects_folder = (
            Path(remote_subjects_folder) if remote_subjects_folder else None
        )
    ...
    def run(self, number_of_expected_devices=None):
        ...
        if (
            self.state == -1
        ):  # this case is not implemented automatically and corresponds to a hard reset
            log.info(f"{self.state}, {self.session_path}")
            shutil.rmtree(self.remote_session_path)
            self.initialize_experiment()
        if (
            self.state == 0
        ):  # the session hasn't even been initialzed: copy the stub to the remote
            log.info(f"{self.state}, {self.session_path}")
            self.initialize_experiment()
    ...
    def get_state(self):
        ...
        """
        if (
            self.remote_subjects_folder is None
            or not self.remote_subjects_folder.exists()
        ):
            return (
                None,
                f"Remote subjects folder {self.remote_subjects_folder} set to Null or unreachable",
            )
        ...
        if status_file is None:
            status_file = self.file_remote_experiment_description.with_suffix(
                ".status_pending"
            )
            status_file.touch()
    ...
    def initialize_experiment(self, acquisition_description=None, overwrite=True):
        ...
        if not self.remote_subjects_folder:
            log.info(
                "The remote path is unspecified and remote experiment.description stub creation is omitted."
            )
        else:
            remote_stub_file = self.file_remote_experiment_description
            previous_description = (
                session_params.read_params(remote_stub_file)
                if remote_stub_file.exists() and not overwrite
                else {}
            )
            try:
                merged_description = session_params.merge_params(
                    previous_description, acquisition_description
                )
                session_params.write_yaml(remote_stub_file, merged_description)
                for f in remote_stub_file.parent.glob(
                    remote_stub_file.stem + ".status_*"
                ):
                    f.unlink()
                ...
                if self.assert_connect_on_init:
                    raise Exception(
                        f"Failed to write data to remote device at: {remote_stub_file}. \n {e}"
                    ) from e
                log.warning(
                    f"Failed to write data to remote device at: {remote_stub_file}. \n {e}"
                )
        ...
        session_params.write_yaml(
            self.file_experiment_description,
            session_params.merge_params(previous_description, acquisition_description),
        )
        log.info(
            f"Written data to local session at : {self.file_experiment_description}."
        )
    ...
    def finalize_copy(self, number_of_expected_devices=None):
        ...
        if number_of_expected_devices is None:
            log.warning(
                f"Number of expected devices is not specified, will not finalize this session {self.session_path}"
            )
            return
        ready_to_finalize = 0
        ...
        for file_stub in files_stub:
            ...
            if (
                next(iter(ad_stub.get("sync", {})), None) != "bpod"
                and number_of_expected_devices == 1
            ):
                log.warning(
                ...
                return
        log.info(
            f"{ready_to_finalize}/{number_of_expected_devices} copy completion status"
        )
        if ready_to_finalize == number_of_expected_devices:
            for file_stub in files_stub:
                session_params.aggregate_device(
                    file_stub,
                    self.remote_session_path.joinpath(
                        "_ibl_experiment.description.yaml"
                    ),
                )
                file_stub.with_suffix(".status_complete").rename(
                    file_stub.with_suffix(".status_final")
                )
...
class VideoCopier(SessionCopier):
    ...
    def create_video_stub(self, nvideos=None):
        match len(list(self.session_path.joinpath("raw_video_data").glob("*.avi"))):
            case 3:
                stub_file = Path(iblrig.__file__).parent.joinpath(
                    "device_descriptions", "cameras", "body_left_right.yaml"
                )
            case 1:
                stub_file = Path(iblrig.__file__).parent.joinpath(
                    "device_descriptions", "cameras", "left.yaml"
                )
    ...
    def initialize_experiment(self, acquisition_description=None, **kwargs):
        if not acquisition_description:
            ...
            acquisition_description = session_params.read_params(
                self.file_experiment_description
            )
        self._experiment_description = acquisition_description
        super().initialize_experiment(
            acquisition_description=acquisition_description, **kwargs
        )
...
class EphysCopier(SessionCopier):
    ...
    def initialize_experiment(
        self, acquisition_description=None, nprobes=None, **kwargs
    ):
        if not acquisition_description:
            nprobes = nprobes or len(
                list(self.session_path.joinpath("raw_ephys_data").rglob("*.ap.bin"))
            )
            match nprobes:
                case 1:
            ...
            stub_file = Path(iblrig.__file__).parent.joinpath(
                "device_descriptions", "neuropixel", stub_name
            )
            sync_file = Path(iblrig.__file__).parent.joinpath(
                "device_descriptions", "sync", "nidq.yaml"
            )
            acquisition_description = session_params.read_params(stub_file)
    ...
    def _copy_collections(self):
        ...
        """
        log.info(
            f"Transferring ephys session: {self.session_path} to {self.remote_session_path}"
        )
        ...
        path_wiring = Path(iblrig.__file__).parent.joinpath(
            "device_descriptions", "neuropixel", "wirings"
        )
        probe_model = "3A"
        for file_nidq_bin in self.session_path.joinpath("raw_ephys_data").glob(
            "*.nidq.bin"
        ):
            probe_model = "3B"
            shutil.copy(
                path_wiring.joinpath("nidq.wiring.json"),
                file_nidq_bin.with_suffix(".wiring.json"),
            )
        for file_ap_bin in self.session_path.joinpath("raw_ephys_data").rglob(
            "*.ap.bin"
        ):
            shutil.copy(
                path_wiring.joinpath(f"{probe_model}.wiring.json"),
                file_ap_bin.with_suffix(".wiring.json"),
            )
        try:
            ...
            log.info(
                "Probe creation failed, please create the probe insertions manually. Continuing transfer..."
            )
        return copy_folders(
