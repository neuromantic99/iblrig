    ...
    def send2bonsai(self, **kwargs):
        ...
        for k in kwargs:
            if k in self.OSC_PROTOCOL:
                ...
                value = (
                    kwargs[k].item() if isinstance(kwargs[k], np.generic) else kwargs[k]
                )
                self.send_message(
                    self.OSC_PROTOCOL[k]["mess"], self.OSC_PROTOCOL[k]["type"](value)
                )
...
class BonsaiRecordingMixin:
    ...
    def start_mixin_bonsai_microphone(self):
        ...
        if (
            self._camera_mixin_bonsai_get_workflow_file(
                self.hardware_settings.get("device_cameras", None)
            )
            is not None
        ):
            return
    ...
    def start_mixin_bonsai_cameras(self):
        ...
        bonsai_camera_file = self.paths.IBLRIG_FOLDER.joinpath(
            "devices", "camera_setup", "setup_video.bonsai"
        )
        self.create_bonsai_layout_from_template(bonsai_camera_file)
        # this locks until Bonsai closes
        cmd = [
            str(self.paths.BONSAI),
            str(bonsai_camera_file),
            "--start-no-debug",
            "--no-boot",
        ]
        self.logger.info("starting Bonsai microphone recording")
...
class BonsaiVisualStimulusMixin:
    ...
    def run_passive_visual_stim(
        self, map_time="00:05:00", rate=0.1, sa_time="00:05:00"
    ):
        file_bonsai_workflow = self.paths.VISUAL_STIM_FOLDER.joinpath(
            "passiveChoiceWorld", "passiveChoiceWorld_passive.bonsai"
        )
        file_output_rfm = self.paths.SESSION_RAW_DATA_FOLDER.joinpath(
        ...
        self.logger.info(" ".join(cmd))
        s = subprocess.run(
            cmd, stdout=subprocess.PIPE, cwd=file_bonsai_workflow.parent
        )  # locking call
