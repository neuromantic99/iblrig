...
def _download_from_alyx_or_flir(asset: int, filename: str, target_md5: str) -> str:
    ...
    if out_file.exists() and hashfile.md5(out_file) == target_md5:
        return out_file
    try:
        tmp_file, md5_sum = AlyxClient().download_file(
    ...
    if md5_sum != target_md5:
        raise Exception(
            f"`{filename}` does not match the expected MD5 - please try running the script again or"
        )
    return out_file
...
def install_spinnaker():
    ...
    print(
        "This script will try to automatically download & install Spinnaker SDK for Windows"
    )
    ...
    file_winsdk = _download_from_alyx_or_flir(
        54386, "SpinnakerSDK_FULL_3.1.0.79_x64.exe", "d9d83772f852e5369da2fbcc248c9c81"
    )
...
def install_pyspin():
    ...
    # Display some information
    print(
        "This script will try to automatically download & install PySpin to the IBLRIG Python environment"
    )
    input("Press [ENTER] to continue.\n")
    ...
    if pyspin_installed():
        if not ask_user(
            "PySpin is already installed. Do you want to continue anyways?"
        ):
            return
    ...
    if pyspin_installed():
        ...
        file_zip = _download_from_alyx_or_flir(
            54396,
            "spinnaker_python-3.1.0.79-cp310-cp310-win_amd64.zip",
        ...
        return_code = subprocess.check_call(
            [sys.executable, "-m", "pip", "install", file_whl]
        )
        if return_code == 0:
