...
def iterate_previous_sessions(subject_name, task_name, n=1, **kwargs):
    ...
    if rig_paths.remote_subjects_folder is not None:
        remote_sessions = _iterate_protocols(
            rig_paths.remote_subjects_folder.joinpath(subject_name),
            task_name=task_name,
            n=n,
        )
...
def _iterate_protocols(subject_folder, task_name, n=1):
    ...
    if subject_folder is None or Path(subject_folder).exists() is False:
        return protocols
    for file_experiment in sorted(
        subject_folder.rglob("_ibl_experiment.description*.yaml"), reverse=True
    ):
        ...
        for ad_task in reversed(ad["tasks"]):
            ...
            task_settings = load_settings(
                session_path, task_collection=adt["collection"]
            )
            if (
                task_settings.get("NTRIALS", 43) < 42
            ):  # we consider that under 42 trials it is a dud session
                continue
            protocols.append(
                Bunch(
                    {
                        "session_stub": "_".join(
                            file_experiment.parent.parts[-2:]
                        ),  # 2019-01-01_001
                        "session_path": file_experiment.parent,
                        "task_collection": adt["collection"],
                        "experiment_description": ad,
                        "task_settings": task_settings,
                        "file_task_data": session_path.joinpath(
                            adt["collection"], "_iblrig_taskData.raw.jsonable"
                        ),
                    }
                )
            )
...
def get_local_and_remote_paths(local_path=None, remote_path=None, lab=None):
    ...
    if paths.local_data_folder is None:
        paths.local_data_folder = (
            Path(p)
            if (p := iblrig_settings["iblrig_local_data_path"])
            else Path.home().joinpath("iblrig_data")
        )
    if paths.remote_data_folder is None:
        paths.remote_data_folder = (
            Path(p) if (p := iblrig_settings["iblrig_remote_data_path"]) else None
        )
    paths.local_subjects_folder = Path(paths.local_data_folder).joinpath(
        lab or iblrig_settings["ALYX_LAB"] or "", "Subjects"
    )
    paths.remote_subjects_folder = (
...
def patch_settings(rs: dict, name: str) -> dict:
    ...
    """
    if name.startswith("hardware") and version.parse(
        rs.get("VERSION", "0.0.0")
...
def get_bonsai_path(use_iblrig_bonsai: bool = True) -> str:
    ...
    bonsai_folder = next(
        (
            folder
            for folder in Path(iblrig_folder).glob("*")
            if folder.is_dir() and "Bonsai" in folder.name
        ),
        None,
    )
...
def iterate_collection(session_path: str, collection_name="raw_task_data") -> str:
    ...
    tasks = sorted(
        filter(re.compile(f"{collection_name}" + "_[0-9]{2}").match, collection_names)
    )
    if len(tasks) == 0:
        return f"{collection_name}_00"
