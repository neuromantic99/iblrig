...
class Session(ChoiceWorldSession):
    ...
    def __init__(self, **kwargs):
        ...
        all_trials = pd.read_parquet(
            Path(__file__).parent.joinpath("passiveChoiceWorld_trials_fixtures.pqt")
        )
        self.trials_table = all_trials[all_trials["session_id"] == SESSION_IDX].copy()
        self.trials_table["reward_valve_time"] = self.compute_reward_time(
            amount_ul=self.trials_table["reward_amount"]
        )
    ...
    def _run(self):
        ...
        self.logger.info(
            "Starting spontaneous activity followed by receptive field mapping"
        )
        ...
        for self.trial_num, trial in self.trials_table.iterrows():
            self.logger.info(
                f"Delay: {trial.stim_delay}; ID: {trial.stim_type}; Count: {self.trial_num}/300"
            )
            sys.stdout.flush()
