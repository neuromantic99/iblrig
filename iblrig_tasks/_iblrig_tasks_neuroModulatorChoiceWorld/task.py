...
class Session(BiasedChoiceWorldSession):
    ...
    extractor_tasks = [
        "TrialRegisterRaw",
        "ChoiceWorldNeuromodulators",
        "TrainingStatus",
    ]

    def __init__(self, *args, **kwargs):
        super(Session, self).__init__(*args, **kwargs)
        self.trials_table["omit_feedback"] = np.zeros(
            self.trials_table.shape[0], dtype=bool
        )
        self.trials_table["choice_delay"] = np.zeros(
            self.trials_table.shape[0], dtype=np.float32
        )

    def next_trial(self):
        super(Session, self).next_trial()
        # then there is a probability of omitting feedback regardless of the choice
        self.trials_table.at[self.trial_num, "omit_feedback"] = (
            np.random.random() < self.task_params.OMIT_FEEDBACK_PROBABILITY
        )

        # then drawing the delay for the choice
        choice_delay_strategy = "binned"
        if (
            choice_delay_strategy == "binary"
        ):  # this is a choice with probabilities 1/3 2/3
            self.trials_table.at[self.trial_num, "choice_delay"] = np.random.choice(
                [1.5, 3.0], p=[2 / 3, 1 / 3]
            )
        elif (
            choice_delay_strategy == "uniform"
        ):  # uniform probability draw between 1.5s and 3s
            self.trials_table.at[self.trial_num, "choice_delay"] = (
                np.random.random() * 1.5 + 1.5
            )
        elif (
            choice_delay_strategy == "binned"
        ):  # 5 valures from 0 to 2.5 secs The "Charline Way"
            self.trials_table.at[self.trial_num, "choice_delay"] = np.random.choice(
                np.linspace(0, 2.5, 3)
            )
        ...
        if self.task_params.VARIABLE_REWARDS:
            ...
            reward_amount = (
                1.5
                if self.block_num == 0
                else np.random.choice(REWARD_AMOUNTS_UL, p=[0.8, 0.2])
            )
    ...
    def get_state_machine_trial(self, i):
        ...
        if i == 0:  # First trial exception start camera
            ...
            self.logger.info(
                "First trial initializing, will move to next trial only if:"
            )
...
class SessionRelatedBlocks(Session):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.trials_table["probability_left_rich"] = np.zeros(
            self.trials_table.shape[0], dtype=np.float32
        )
        self.blocks_table["probability_left_rich"] = np.zeros(
            self.blocks_table.shape[0], dtype=np.float32
        )
    ...
    def new_block(self):
        ...
        self.blocks_table.at[
            self.block_num, "probability_left_rich"
        ] = probability_left_rich

    def next_trial(self):
        super(SessionRelatedBlocks, self).next_trial()
        self.trials_table.at[
            self.trial_num, "reward_amount"
        ] = self.draw_reward_amount()
