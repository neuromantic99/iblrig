...
for description, port in ports.items():
    ...
    try:
        ...
        log_fun(
            "fail",
            f"cannot connect to {port} ({description}) - is another process using the port?",
            last=True,
        )
        continue
    else:
    ...
    match description:
        ...
        case "COM_ROTARY_ENCODER":
            ...
            ok = len(query(s, b"Q", 2)) > 1 and query(s, b"P00", 1) == (1).to_bytes(
                1, "little"
            )
        case _:
    ...
    if ok:
        ...
        log_fun(
            "fail",
            f"device on port {port} does not appear to be a {device_name}",
            last=True,
        )
...
if "COM_ROTARY_ENCODER" in ports.keys():
    ...
    log_fun(
        "warn", "please move the wheel to the left (animal's POV) by a quarter turn"
    )
    while np.abs(p) < 200:
        p = np.frombuffer(query(s, b"Q", 2), dtype=np.int16)[0]
    if p > 0:
        log_fun(
            "fail",
            "Rotary encoder seems to be wired incorrectly - try swapping A and B",
            last=True,
        )
    else:
    ...
    match hw_settings["device_sound"]["OUTPUT"]:
        case "harp":
            ...
            if not dev:
                ...
                log_fun(
                    "pass",
                    f"found USB device {dev.idVendor:04X}:{dev.idProduct:04X} (Harp Sound Card)",
                )
            ...
            if not dev:
                ...
                log_fun(
                    "pass",
                    f"found USB device {dev.vid:04X}:{dev.pid:04X} (FT232 UART), serial port: {dev.name}",
                )

            module = [m for m in modules if m.name.startswith("SoundCard")]
            if len(module) == 0:
                log_fun(
                    "fail", "Harp Sound Card is not connected to the Bpod", last=True
                )
            elif len(module) > 1:
                log_fun(
                    "fail",
                    "more than one Harp Sound Card connected to the Bpod",
                    last=True,
                )
            else:
                log_fun(
...
if module:
    log_fun(
        "pass",
        f'module "{module.name}" is connected to the Bpod\'s module port #{module.serial_port}',
    )
...
if "device_cameras" in hw_settings and isinstance(hw_settings["device_cameras"], dict):
    ...
    triggers = [
        i.host_timestamp
        for i in bpod.session.current_trial.events_occurrences
        if i.content == "Port1In"
    ]
    ...
    if len(triggers) == 0:
        ...
        log_fun(
            "info",
            f"average frame-rate: {np.mean(np.diff(triggers)) * 1E3:.3f} Hz",
            last=True,
        )
